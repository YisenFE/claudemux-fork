...
| `.send-token` | `sendTokenFile` | Latest `tm send`'s claim stamp (ms), max-wins; drives send auto-supersede (decision [send-supersede](/.agents/decisions/send-supersede.md)) | `tm` (`tm send`, Claude) | `tm` (an in-flight `tm send`'s wait loop) |
