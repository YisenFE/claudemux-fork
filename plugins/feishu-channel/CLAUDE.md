...
- **Replies route by `chat_id` by default; a topic reply adds an optional `message_id` anchor.** An arbitrary `message_id` echoed back from some other context cannot redirect a reply into an unrelated chat. When an inbound message carried a `thread_id` (it came from a Feishu topic), `reply` takes that message's `message_id` and threads the answer into the topic via `im.message.reply(reply_in_thread)`; a chat that does not support thread replies (Feishu `230071`) falls back to the `chat_id` send.
