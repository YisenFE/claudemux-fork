...
- **Replies route by `chat_id` by default; a model-supplied `message_id` threads into that message's topic.** When an inbound message carried a `thread_id` (it came from a Feishu topic), Claude copies that tag's `message_id` to `reply` and the answer is threaded into the topic via `im.message.reply(reply_in_thread)`; without it the reply goes to `chat_id` as a normal message. Honoring the model-supplied `message_id` with no server-side attribution check is a deliberate decision — Claude decides "answer in this topic" vs "answer the chat" the same way it already decides a p2p vs a group reply — so the channel stays stateless and can thread onto any visible message. A chat that does not support thread replies (Feishu `230071`) falls back to the `chat_id` send. → `.agents/decisions/feishu-channel-event-registry.md`.
