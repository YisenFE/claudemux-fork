...
Sending a teammate a **second `tm send` while an earlier one is still waiting is supported** — it is the "guide the model" pattern. The earlier send auto-supersedes: it returns early with exit 0 and a `superseded …` note (stderr) saying its prompt was delivered and its result merges into the later send's turn. Only the latest send keeps waiting and prints the combined reply. This covers Claude `tm send`, and `tm spawn --prompt` / `tm resume --prompt` (which route through the same send path). See [decision send-supersede](/.agents/decisions/send-supersede.md). To fire a steering follow-up you do not have to wait for the first send to return — issue the next `tm send` and read the merged reply from it (or `tm wait <name>`).
