...
## A second `tm send` steers; other mid-flight input still breaks the wait

Sending a teammate a **second `tm send` while an earlier one is still waiting is supported** — it is the "guide the model" pattern. The earlier send auto-supersedes: it returns early with exit 0 and a `superseded …` note (stderr) saying its prompt was delivered and queued into the teammate's current run. Only the latest send keeps waiting for the result. This covers Claude `tm send`, and `tm spawn --prompt` / `tm resume --prompt` (which route through the same send path). See [decision send-supersede](/.agents/decisions/send-supersede.md). To fire a steering follow-up you do not have to wait for the first send to return — issue the next `tm send` and read the result from it.

Whether the queued prompt is **merged** depends on the teammate's run (live-repro finding in the decision record): when the steering send lands at a mid-task pause (e.g. the teammate is running a tool), Claude Code folds it into the same turn and the surviving send prints the combined reply. But on a pure-generation turn with no mid-task pause, the queued prompt runs as a **separate** turn — and the surviving send can even come back empty (the queued send's `clearIdle` wiped the `.last` baseline). So do not assume the survivor's stdout always carries both prompts' answers; when it looks short or empty, recover the real reply with `tm wait <name>` / `tm last <name>` (the content is in the transcript regardless).

**Other mid-flight input is still not covered** and still breaks `tm`'s Stop-signal capture — do not aim any of these at a teammate while a `tm send` / `tm spawn --prompt` / `tm wait` is tracking it: `/reload-plugins`, `tm reload <name>`, `tm compact <name>`, or raw keystrokes. None of them participate in supersede, so the tracking call's `/tmp/claude-idle/<sid>.last` never settles for the turn it expected and it can hang to the timeout. `/reload-plugins` is the sharpest version because it reloads the Stop hook itself while a wait depends on it. A plain `tm wait` is also not superseded by a later send (it claims no send token), so it is the one readback that a steering send can still strand — prefer `tm send` for the follow-up, or re-collect with a fresh `tm wait` afterward.

Before sending any of that non-`send` input to a teammate, check whether a background `tm` call is still tracking it (the unfinished task notification, plus `tm states` if needed). If one is, wait for it to return (or read the artifact directly) first.
